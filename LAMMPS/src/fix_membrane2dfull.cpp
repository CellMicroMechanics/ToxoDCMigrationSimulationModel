...
FixMembrane2dFull::FixMembrane2dFull(LAMMPS *lmp, int narg, char **arg) : Fix(lmp, narg, arg)
{
  ...
  KP = utils::numeric(FLERR, arg[9], false, lmp);    // prefered perimeter
  P0 = utils::numeric(FLERR, arg[10], false, lmp);   // prefered perimeter
  if (narg == 12) nevery = utils::numeric(FLERR, arg[11], false, lmp);
  // line tension
  tension0 = 0;
  tension_grad0 = 0;
  tension_flag = 0;
  if (narg > 12) {
    ...
    double px0 = utils::numeric(FLERR, arg[11], false, lmp);
    double py0 = utils::numeric(FLERR, arg[12], false, lmp);
    double pz0 = utils::numeric(FLERR, arg[13], false, lmp);
    // normalize polarity vector;
    double lp = sqrt(pow(px0, 2) + pow(py0, 2) + pow(pz0, 2));
    px = px0 / lp;
    py = py0 / lp;
    pz = pz0 / lp;
    rate = utils::numeric(FLERR, arg[14], false, lmp);    // add-atom rate
    seed = utils::numeric(FLERR, arg[15], false, lmp);    // random seed
    nevery = utils::numeric(FLERR, arg[16], false, lmp);
    if (rate != 0) isPolymerisationEnabled = true;
    rng = new RanMars(lmp, seed);
    force_reneighbor = 1;

    if (narg > 17) {
      tension0 = utils::numeric(FLERR, arg[17], false, lmp);
      tension_grad0 = utils::numeric(FLERR, arg[18], false, lmp);
      tension_flag = 1;
    }
  }
...
void FixMembrane2dFull::findMaxEdgeLengthAndTag()
{
...
void FixMembrane2dFull::findMinEdgeLengthAndTag()
{
...
void FixMembrane2dFull::applyMembraneDynamics(tagint tag1, tagint tag2, int mode_flag)
{
...
void FixMembrane2dFull::initializeMembraneBonds2D()
{
...
void FixMembrane2dFull::setup(int)
{
  atom->map_init(1);
  atom->map_set();
  getCOM();
...
void FixMembrane2dFull::updatePredefinedTension()
{
...
void FixMembrane2dFull::computeTensionEnergy()
{
  double **x = atom->x;
...
void FixMembrane2dFull::pre_exchange()
{
  isChanged = 0;
  ...
  double rate_depoly = rate;
    //  rate_lo + (rate_hi - rate_lo) / (1 + exp(-alpha * (Etension / Etension0 - 1)));
  // if(comm->me == 0)
  // {
...
void FixMembrane2dFull::pre_force(int)
{
  // incrementing age
  if (atom->age != nullptr) {
    for (int i = 0; i < atom->nlocal; i++) {
  ...
  if (KP != 0)
  {
    ...
    for(int i = 0; i < nlocal; i++)
    {
        if(atom->mask[i] & groupbit )
        {
            ...
            if(nextI != -1)
            {
                double dx = atom->x[nextI][0] - atom->x[thisI][0];
                double dy = atom->x[nextI][1] - atom->x[thisI][1];
                double dz = 0;    //atom->x[nextI][2] - atom->x[thisI][2];
                domain->minimum_image(dx, dy, dz);
                currentPerimeter += sqrt(dx*dx + dy*dy + dz*dz);
            }
        }
    }
    MPI_Allreduce(MPI_IN_PLACE, &currentPerimeter, 1, MPI_DOUBLE, MPI_SUM, world);
    double dP = currentPerimeter - P0;
    double fP = KP * dP;
    ...
    for(int i = 0; i < nlocal; i++)
    {
        if(atom->mask[i] & groupbit)
        {
            ...
            if(prevI != -1 && nextI != -1)
            {
                double dx1 = atom->x[thisI][0] - atom->x[prevI][0];
                double dy1 = atom->x[thisI][1] - atom->x[prevI][1];
                double dz1 = 0;    //atom->x[nextI][2] - atom->x[prevI][2];
                
                double dx2 = atom->x[thisI][0] - atom->x[nextI][0];
                double dy2 = atom->x[thisI][1] - atom->x[nextI][1];
                double dz2 = 0;    //atom->x[nextI][2] - atom->x[thisI][2];
                ...
                double fx = fP * (dx1/l1 + dx2/l2);
                double fy = fP *(dy1/l1 + dy2/l2);
                f[thisI][0] -= fx;
                f[thisI][1] -= fy;
            }
        }
...
void FixMembrane2dFull::post_force(int)
{
  double **f = atom->f;
  int nlocal = atom->nlocal;
  for (int i = 0; i < nlocal; i++) {
...
void FixMembrane2dFull::end_of_step()
{
  getCOM();    // this is called nevery steps for efficiency since COM does not change drastically,
}

int FixMembrane2dFull::pack_reverse_comm(int n, int first, double *buff)
{
  int i, m = 0;
...
void FixMembrane2dFull::unpack_reverse_comm(int n, int *list, double *buff)
{
  int i, j, m = 0;
  for (i = 0; i < n; i++) {
    j = list[i];
