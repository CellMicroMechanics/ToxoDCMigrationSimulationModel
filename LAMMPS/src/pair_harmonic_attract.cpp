...
#include "pair_harmonic_attract.h"
...
PairHarmonicAttract::PairHarmonicAttract(LAMMPS *lmp) : Pair(lmp), k(nullptr), cut(nullptr)
{
  writedata = 1;
}
...
PairHarmonicAttract::~PairHarmonicAttract()
{
  if (allocated) {
...
void PairHarmonicAttract::compute(int eflag, int vflag)
{
  ...
  for (ii = 0; ii < inum; ii++) {
    ...
    for (jj = 0; jj < jnum; jj++) {
      ...
      if (rsq < cutsq[itype][jtype]) {
        ...
        double fpair = 2.0 * prefactor / r;
        if(r <= r0sq[itype][jtype]) fpair = 0;
        fxtmp += delx * fpair;
        fytmp += dely * fpair;
        fztmp += delz * fpair;
...
void PairHarmonicAttract::allocate()
{
  allocated = 1;
  int n = atom->ntypes + 1;
  ...
  memory->create(cut, n, n, "pair:r0sq");
...
void PairHarmonicAttract::settings(int narg, char **arg)
{
...
void PairHarmonicAttract::coeff(int narg, char **arg)
{
...
double PairHarmonicAttract::init_one(int i, int j)
{
  if (setflag[i][j] == 0) {
...
void PairHarmonicAttract::write_restart(FILE *fp)
{
...
void PairHarmonicAttract::read_restart(FILE *fp)
{
...
void PairHarmonicAttract::write_restart_settings(FILE *fp)
{
...
void PairHarmonicAttract::read_restart_settings(FILE *fp)
{
  int me = comm->me;
  if (me == 0) {
...
void PairHarmonicAttract::write_data(FILE *fp)
{
...
void PairHarmonicAttract::write_data_all(FILE *fp)
{
...
void *PairHarmonicAttract::extract(const char *str, int &dim)
{
  dim = 2;
