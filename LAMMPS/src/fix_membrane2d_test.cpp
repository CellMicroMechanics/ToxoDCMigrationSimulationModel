...
FixMembrane2dTest::FixMembrane2dTest(LAMMPS *lmp, int narg, char **arg) : Fix(lmp, narg, arg)
{
  ...
  tension_grad0 = 0;
  tension_flag = 0;
  depolymerization_phase = 1;
  depolymerization_phase_gap = 0;
  ...
  if (narg > 10) {
    ...
    if (narg > 15) {
      ...
      tension_flag = 1;
      if (narg > 17) {
        depolymerization_phase = utils::numeric(FLERR, arg[17], false, lmp); // depolymerization phase: n1*nevery
        depolymerization_phase_gap = utils::numeric(FLERR, arg[18], false, lmp); // depolymerization phase gap n_gap*nevery
      }
    }
  }
...
void FixMembrane2dTest::findMaxEdgeLengthAndTag()
{
...
void FixMembrane2dTest::findMinEdgeLengthAndTag()
{
...
void FixMembrane2dTest::applyMembraneDynamics(tagint tag1, tagint tag2, int mode_flag)
{
...
void FixMembrane2dTest::initializeMembraneBonds2D()
{
...
void FixMembrane2dTest::setup(int)
{
  atom->map_init(1);
  atom->map_set();
  getCOM();
...
void FixMembrane2dTest::updatePredefinedTension()
{
  ...
  // scale is kept for future extension, i.e., mechanosensing
...
void FixMembrane2dTest::computeTensionEnergy()
{
  double **x = atom->x;
...
void FixMembrane2dTest::pre_exchange()
{
  isChanged = 0;
  ...
  double rate_depoly;
  if (depolymerization_phase_gap != 0) {
    bool in_dphase_gap = false;
    double tpos = (update->ntimestep - init_update)  % ((depolymerization_phase + depolymerization_phase_gap)*nevery);
    if ( tpos  <=
        depolymerization_phase_gap*nevery) {
      in_dphase_gap = true;
    }
    if (in_dphase_gap)
      rate_depoly = 0;
      ...
      rate_depoly =
          rate * (depolymerization_phase + depolymerization_phase_gap) / depolymerization_phase;
  }
  else
    {
      computeTensionEnergy();
      rate_depoly = 0.5*rate + 1.5*rate / (1 + exp(-20 * (Etension / Etension0 - 1)));
    }
  
  // if(comm->me == 0) cout << rate_depoly << endl;
  double prob1 =
...
void FixMembrane2dTest::pre_force(int)
{
  // incrementing age
  if (atom->age != nullptr) {
    for (int i = 0; i < atom->nlocal; i++) {
...
void FixMembrane2dTest::post_force(int)
{
  double **f = atom->f;
  int nlocal = atom->nlocal;
  for (int i = 0; i < nlocal; i++) {
...
void FixMembrane2dTest::end_of_step()
{
  getCOM();    // this is called nevery steps for efficiency since COM does not change drastically,
}

int FixMembrane2dTest::pack_reverse_comm(int n, int first, double *buff)
{
  int i, m = 0;
...
void FixMembrane2dTest::unpack_reverse_comm(int n, int *list, double *buff)
{
  int i, j, m = 0;
  for (i = 0; i < n; i++) {
    j = list[i];
